...
# Loading files
general_data <- read.csv("Data/general_data.csv", stringsAsFactors = F)
survey_data <- read.csv("Data/employee_survey_data.csv", stringsAsFactors = F)
manager_survey_data <- read.csv("Data/manager_survey_data.csv", stringsAsFactors = F)
in_time <- read.csv("Data/in_time.csv", stringsAsFactors = F)
out_time <- read.csv("Data/out_time.csv", stringsAsFactors = F)
...
plot1 <- plot_ly(x=final_data$Attrition, y=final_data$Age, color = final_data$Attrition, type = "box") %>% 
        layout(xaxis= list(title= "Attrition"), yaxis= list(title= "Age"))
plot2 <- plot_ly(x=final_data$Attrition, y=final_data$TotalWorkingYears, color = ~final_data$Attrition, type = "box") %>% 
         layout(xaxis= list(title= "Attrition"), yaxis= list(title= "TotalWorkingYears"))
group1<- subplot(plot1, plot2, titleX = TRUE, titleY = TRUE, margin = 0.05)
group1

plot3 <- plot_ly(x=final_data$Attrition, y=final_data$MonthlyIncome, color = ~final_data$Attrition, type = "box") %>% 
        layout(xaxis= list(title= "Attrition"), yaxis= list(title= "Monthly Income"))
plot4 <- plot_ly(x=final_data$Attrition, y=final_data$NumCompaniesWorked, color = ~final_data$Attrition, type = "box") %>% 
        layout(xaxis= list(title= "Attrition"), yaxis= list(title= "Companies Worked"))
plot5 <- plot_ly(x=final_data$Attrition, y=final_data$PercentSalaryHike, color = ~final_data$Attrition, type = "box") %>% 
         layout(xaxis= list(title= "Attrition"), yaxis= list(title= "Percent Salary Hike"))
group2<- subplot(plot3, plot4, plot5, titleX = TRUE, titleY = TRUE, margin = 0.05)
group2


plot6 <- plot_ly(x=final_data$Attrition, y=final_data$StandardHours, color = ~final_data$Attrition, type = "box") %>% 
  layout(xaxis= list(title= "Attrition"), yaxis= list(title= "Standar dHours"))
plot7 <- plot_ly(x=final_data$Attrition, y=final_data$TotalWorkingYears, color = ~final_data$Attrition, type = "box") %>% 
  layout(xaxis= list(title= "Attrition"), yaxis= list(title= "Total Working Years"))
plot8 <- plot_ly(x=final_data$Attrition, y=final_data$TrainingTimesLastYear, color = ~final_data$Attrition, type = "box") %>% 
  layout(xaxis= list(title= "Attrition"), yaxis= list(title= "Training Times Last Year"))
group3<- subplot(plot6, plot7, plot8, titleX = TRUE, titleY = TRUE, margin = 0.05)
group3
       
plot9 <- plot_ly(x=final_data$Attrition, y=final_data$YearsSinceLastPromotion, color = ~final_data$Attrition, type = "box") %>% 
  layout(xaxis= list(title= "Attrition"), yaxis= list(title= "Years Since Last Promotion"))
plot10 <- plot_ly(x=final_data$Attrition, y=final_data$YearsWithCurrManager, color = ~final_data$Attrition, type = "box") %>% 
  layout(xaxis= list(title= "Attrition"), yaxis= list(title= "Years With Current Manager"))
group4<- subplot(plot9, plot10, titleX = TRUE, titleY = TRUE, margin = 0.05)
group4

plot11 <- plot_ly(x=final_data$Attrition, y=final_data$YearsAtCompany, color = ~final_data$Attrition, type = "box") %>% 
  layout(xaxis= list(title= "Attrition"), yaxis= list(title= "Years At Company"))
plot12 <- plot_ly(x=final_data$Attrition, y=final_data$AverageWorkTime, color = ~final_data$Attrition, type = "box") %>% 
  layout(xaxis= list(title= "Attrition"), yaxis= list(title= "Average Work Time"))
group5<- subplot(plot11, plot12, titleX = TRUE, titleY = TRUE, margin = 0.05)
group5
#############################################################################
...
plotb1 <- ggplot(final_data, aes(x=BusinessTravel,fill=Attrition), align = "h") + geom_bar(position = position_fill()) +  labs(y="Proportion") + bar_theme1
plotb2 <- ggplot(final_data, aes(x=Department,fill=Attrition), align = "h") + geom_bar(position = position_fill())+ labs(y="Proportion") + bar_theme1
sub1 <- subplot(plotb1, plotb2)
sub1 <- ggplotly(sub1)
sub1

plotb3 <- ggplot(final_data, aes(x=Education,fill=Attrition), align = "h") + geom_bar(position = position_fill()) +  labs(y="Proportion") + bar_theme1
plotb4 <- ggplot(final_data, aes(x=JobRole,fill=Attrition), align = "h") + geom_bar(position = position_fill())+ labs(y="Proportion") + bar_theme1
sub2 <- subplot(plotb3, plotb4)
sub2 <- ggplotly(sub2)
sub2

plotb5 <- ggplot(final_data, aes(x=MaritalStatus,fill=Attrition), align = "h") + geom_bar(position = position_fill()) +  labs(y="Proportion") + bar_theme1
plotb6 <- ggplot(final_data, aes(x=StockOptionLevel,fill=Attrition), align = "h") + geom_bar(position = position_fill())+ labs(y="Proportion") + bar_theme1
sub3 <- subplot(plotb5, plotb6)
sub3 <- ggplotly(sub3)
sub3

plotb7 <- ggplot(final_data, aes(x=EnvironmentSatisfaction,fill=Attrition), align = "h") + geom_bar(position = position_fill()) +  labs(y="Proportion") + bar_theme1
plotb8 <- ggplot(final_data, aes(x=JobSatisfaction,fill=Attrition), align = "h") + geom_bar(position = position_fill())+ labs(y="Proportion") + bar_theme1
sub4 <- subplot(plotb7, plotb8)
sub4 <- ggplotly(sub4)
sub4

plotb9 <- ggplot(final_data, aes(x=WorkLifeBalance,fill=Attrition), align = "h") + geom_bar(position = position_fill()) +  labs(y="Proportion") + bar_theme1
plotb10 <- ggplot(final_data, aes(x=JobInvolvement,fill=Attrition), align = "h") + geom_bar(position = position_fill())+ labs(y="Proportion") + bar_theme1
sub5 <- subplot(plotb9, plotb10)
sub5 <- ggplotly(sub5)
sub5

plotb11 <- ggplot(final_data, aes(x=PerformanceRating,fill=Attrition), align = "h") + geom_bar(position = position_fill()) +  labs(y="Proportion") + bar_theme1
plotb12 <- ggplot(final_data, aes(x=Over18,fill=Attrition), align = "h") + geom_bar(position = position_fill())+ labs(y="Proportion") + bar_theme1
sub6 <- subplot(plotb11, plotb12)
sub6 <- ggplotly(sub6)
sub6
